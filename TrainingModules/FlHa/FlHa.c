/* EasyCASE V6.5 02/11/2015 10:28:51 */
/* EasyCASE O
If=horizontal
LevelNumbers=no
LineNumbers=no
Colors=16777215,0,12582912,12632256,0,0,0,16711680,8388736,0,33023,32768,0,0,0,0,0,32768,12632256,255,65280,255,255,16711935
ScreenFont=System,,100,1,-13,0,700,0,0,0,0,0,0,1,2,1,34,96,96
PrinterFont=Courier,,100,1,-85,0,400,0,0,0,0,0,0,1,2,1,49,600,600
LastLevelId=2 */
/* EasyCASE ( 1 */
#include "FlHa.h"
#include "../../Console.h"

/*test*/
/* EasyCASE ( 2
   FlHa_vPrintMe */
/* EasyCASE F */
void FlHa_vPrintMe(void)
   {
           Console_vPrint("FlHa: Will come who will be coming and what will happen will have to happen!\n");
   //testtesttest askljdhaklsjf
//<<<<<<< HEAD
//conflict2
//=======
   //conflict 3
//>>>>>>> master
   }
/* EasyCASE ) */
/* EasyCASE ) */
